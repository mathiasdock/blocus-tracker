...
4. **CSS variables only for colors.** Use `var(--bt-*)` and the accent green `#14B885` family. No new colors. Dark mode works automatically. → `docs/UI.md`
   **Deux exceptions, toutes deux voulues et documentées :** la mascotte garde ses couleurs de pelage naturelles, et les *accents de famille* (`--bt-fam-*`, 5 teintes fermées) distinguent les familles de badges et les rubriques du profil. Demandé par Mathias le 2026-09-09 : les badges tout-verts ne se distinguaient plus les uns des autres. Ne pas les re-verdir. → `DESIGN.md` § Family Accents
