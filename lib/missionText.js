...
const weeklyUnit = (t, id, count) => id === "w_days"
  ? t(count === 1 ? "xp.weeklyDay" : "xp.weeklyDays")
  : t(count === 1 ? "xp.weeklyCourse" : "xp.weeklyCourses");
...
export function weeklyProgressLabel(t, row) {
  ...
  const unit = weeklyUnit(t, id, target);
...
export function weeklyRemaining(t, row) {
  ...
  const unit = weeklyUnit(t, id, left);
  return `${left} ${unit}`;
}
