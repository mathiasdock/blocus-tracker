...
import { AUTOMATIONS } from "./pushAutomations";
...
const PUSH = () => ({
  audience: { total: 212, messageable: 148 },
  universities: UNIS.map((name, index) => ({ name, members: 3 + ((index * 5) % 11) })).sort((a, b) => a.name.localeCompare(b.name, "fr")),
  history: [
    { id: "n-1", title: "Rappel : examen demain", body: "Une dernière révision aujourd'hui, et tu seras prêt·e.", scheduledFor: null, canceled: false, sentAt: NOW - 20 * HOUR, successful: 11, failed: 0, segments: [], targeted: 12, url: "/planning" },
    { id: "n-2", title: "Bonne session d'examens", body: "Les salles de cours de ton établissement sont ouvertes.", scheduledFor: NOW + 2 * DAY, canceled: false, sentAt: null, successful: 0, failed: 0, segments: ["Total Subscriptions"], targeted: 0, url: null },
    { id: "n-3", title: "Ta série est en danger 🔥", body: "Tu n'as pas encore étudié aujourd'hui. Garde ta série !", scheduledFor: null, canceled: false, sentAt: NOW - 44 * HOUR, successful: 23, failed: 2, segments: [], targeted: 25, url: "/dashboard" },
    { id: "n-4", title: "Nouveau : les salles de cours", body: "Retrouve les étudiants qui suivent les mêmes cours que toi.", scheduledFor: null, canceled: true, sentAt: null, successful: 0, failed: 0, segments: ["Total Subscriptions"], targeted: 0, url: "/communautes" },
  ],
});
...
export async function offlineAdminApi(path, { method = "GET", body } = {}) {
  ...
  if (path === "/api/admin/push") {
    ...
    if (method === "POST") return { data: { ok: true, recipients: body?.target?.type === "all" ? 148 : (body?.target?.userIds || []).length || 9 }, error: null };
    ...
    return {
      data: {
        automations: AUTOMATIONS.map((a, index) => ({
          key: a.key, label: a.label, trigger: a.trigger, vars: a.vars,
