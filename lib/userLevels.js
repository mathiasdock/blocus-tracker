import { computeStreak, computeBestStreak } from "./format";
import { computeEarnedBadgeIds, totalBadgeXP } from "./badges";
...
async function computeUserLevelMap(supabase, ids, options = {}) {
  // Production source of truth. The RPC returns only public progression data,
  // so every screen sees the same level regardless of the viewer's RLS access
  // to sessions, objectives or badge rows.
  try {
    const { data: canonicalRows, error: canonicalError } = await supabase.rpc(
      "get_gamification_levels",
      { p_user_ids: ids }
    );
    if (!canonicalError && Array.isArray(canonicalRows)) {
      const canonical = {};
      canonicalRows.forEach((row) => {
        const info = getLevelInfo(Number(row.total_xp || 0));
        canonical[row.user_id] = {
          ...info,
          streak: Number(row.streak || 0),
          // Détail des sources (migration v50), pour la page « Ma progression ».
          // Purement additif : les écrans qui n'en veulent pas l'ignorent, et
          // les recalculer en JavaScript relirait sessions, objectifs, examens
          // et badges à chaque ouverture — le serveur les a déjà en main.
          breakdown: {
            study: Number(row.study_xp || 0),
            objectives: Number(row.objective_xp || 0),
            streak: Number(row.streak_xp || 0),
            exams: Number(row.exam_xp || 0),
            badges: Number(row.badge_xp || 0),
            missions: Number(row.mission_xp || 0),
            referrals: Number(row.referral_xp || 0),
          },
        };
      });
      if (ids.every(id => canonical[id])) return canonical;
    }
  } catch (_) {
    // Offline mode or a deployment where v28 is not applied yet.
  }
  ...
  const [
    ...
    // Publications et réactions ne se comptent plus sur les lignes vivantes :
    // elles sont supprimées au bout de 24 h (migration v46). Le total à vie
    // vit dans user_activity_totals, dont la RLS ne laisse lire que SA propre
    // ligne — un compteur de publications n'a pas à être public.
    // Pour les AUTRES comptes on ne demande donc rien : leurs badges viennent
    // de `user_badges`, qui fait autorité et que l'union plus bas préserve.
    options.selfUserId && ids.includes(options.selfUserId)
      ? supabase.from("user_activity_totals")
        .select("lifetime_posts, lifetime_reactions")
        .eq("user_id", options.selfUserId)
        .maybeSingle()
    ...
    options.includeSelfReferralStats && options.selfUserId && ids.includes(options.selfUserId)
      ...
      : Promise.resolve({ data: null, error: null }),
    supabase.from("streak_freeze_days").select("user_id, used_on").in("user_id", ids),
  ]);
  ...
  // Jours gelés (gel de série, v29) — silencieux si la migration n'est pas
  // encore passée (pas de warn : l'absence de table est un état normal).
  const frozenByUser = {};
  if (!freezeRes?.error) {
    (freezeRes?.data || []).forEach((row) =>
      addToList(frozenByUser, row.user_id, String(row.used_on).slice(0, 10)));
  }
  ...
  // Uniquement pour soi : voir le commentaire sur la requête plus haut.
  const postCounts = {};
  const reactionCounts = {};
  if (!activityTotalsRes?.error && activityTotalsRes?.data && options.selfUserId) {
    postCounts[options.selfUserId] = activityTotalsRes.data.lifetime_posts || 0;
    reactionCounts[options.selfUserId] = activityTotalsRes.data.lifetime_reactions || 0;
  }
  ...
  ids.forEach((id) => {
    ...
    const totalSeconds = sessions.length
      ...
      : fallbackSeconds;
    const streak = computeStreak(sessions, frozenByUser[id] || []);
    const bestStreak = computeBestStreak(sessions, frozenByUser[id] || []);
    const dayTotals = {};
    sessions.forEach((s) => {
    ...
    const badgeIds = [...new Set([...existingBadges, ...earnedBadges])];
    ...
    // Le détail des sources aussi en repli. Il est composé des mêmes termes
    // que le total juste au-dessus : ne le produire que par le RPC laisserait
    // la page « Ma progression » sans son tableau hors-ligne, ou le jour où le
    // serveur ne répond pas — alors que tout est déjà calculé ici.
    // `missions` et `referrals` ne sont pas distinguables de ce côté :
    // bonus_xp les additionne. Ils sont donc regroupés sous « missions », que
    // le RPC sépare quand il répond.
    const breakdown = {
      study: Math.floor(totalSeconds / 60),
      objectives: (completedObjectiveCounts[id] || 0) * 20,
      streak: (bestStreak || 0) * 10,
      exams: (examCounts[id] || 0) * 15,
      badges: totalBadgeXP(badgeIds),
      missions: profiles[id]?.bonus_xp || 0,
      referrals: 0,
    };
    // `streak` exposé en plus (purement additif) : le watcher global s'en sert
    // pour célébrer les paliers de série sans relancer de requête (les sessions
    // sont déjà chargées ici). Les autres consommateurs l'ignorent.
    out[id] = { ...getLevelInfo(totalXP), streak, breakdown };
  });
