...
// ── « Cette semaine » ─────────────────────────────────────────────────────────
// UNE définition dans toute l'app : du lundi 00:00 à maintenant, dans le
// calendrier local de l'appareil. Une fenêtre glissante de 7 jours existe
// ailleurs, mais elle s'appelle « 7 derniers jours ».

/** Les sept dates locales (lundi → dimanche) de la semaine en cours. */
...
/** Secondes étudiées cette semaine (lundi → aujourd'hui). */
export function thisWeekSeconds(rows, now = new Date()) {
  const dates = currentWeekDates(now);
  const from = dates[0];
  const to = localDateOf(now);
  let total = 0;
  for (const row of rows || []) {
    if (row.local_date >= from && row.local_date <= to) total += Number(row.seconds) || 0;
  }
  return total;
}
