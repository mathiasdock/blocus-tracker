...
- Lu / non lu attaché au compte : `notification_reads` (une ligne par notification ouverte) + `notification_inbox_state` (date butoir de « Tout marquer comme lu »), écrits seulement par `notification_mark_read` / `notification_mark_all_read`. Lu sur le téléphone = lu sur l'ordinateur ; une conversation lue dans Messages l'est aussi dans la cloche, un nouveau message la rallume. Les annonces masquées sur l'appareil par l'ancienne cloche sont reprises une fois comme lectures du compte.
- Interface (`components/NotificationCenter.js`) : base « Notification Panel » de Layro via 21st.dev (1 crédit utilisé, 1 gardé), reconstruite sur les tokens Blocus. Ordinateur : panneau accroché à la cloche de la barre latérale (Échap, clic dehors, flèches, focus rendu à la cloche). Téléphone : la feuille `InboxSheet` des demandes d'amis, avec « Tout marquer comme lu » sous le titre. Aujourd'hui / Plus tôt, 20 lignes puis « Voir plus », avatar + pastille de type, point de non-lu + gras + texte pour lecteur d'écran. Chaque ligne ouvre sa destination et devient lue ; le Fil sait maintenant ouvrir `?post=<id>`, même plus vieux que 24 h.
- Retirés : le parrainage codé en dur dans la cloche (`PRODUCT_ANNOUNCEMENTS`, clés `notif.newFeature` / `notif.productReferral`), le masquage d'annonces par appareil, les boutons Accepter / Refuser dans la cloche (la ligne mène aux demandes, où ils sont). Les 4 annonces existantes n'ont pas été touchées. L'onglet Fil ne compte plus que les nouvelles publications (commentaires et réactions sont dans la cloche).
- Requêtes : tous les compteurs de navigation en UN appel (`notification_summary`) toutes les 2 minutes au lieu de 7 à 13 ; changer de page ne relance plus la lecture ni l'abonnement temps réel. `Avatar` sort de Layout (`components/Avatar.js`, réexporté).
- Vérifié : `supabase/tests/notification_center.sql` 38/38 sur la base réelle (transaction annulée) ; 19 nouveaux tests Node (`tests/notification-center.test.mjs`) ; lint ; build ; commit exact rejoué dans une copie propre ; captures hors ligne (jeux `?bt_notif=demo|few|empty|many|long`) desktop/mobile, clair/sombre, FR/EN. Aucun push réel.

## 2026-09-24 — Claude Code — Notifications V1 : le catalogue final

- Supprimées : « relance planning » et « relance étude » (alternance artificielle) et l'ancienne « relance des nouveaux ». Leurs envois passés restent lisibles dans l'historique et comptent encore dans le plafond.
- Rappel du soir (`lib/eveningPlan.mjs`, logique pure) : une notification au plus par membre et par soir, dans son fuseau, la première qui s'applique — 1. examen demain (nom + heure, plusieurs examens = une seule notification résumée), 2. série en danger (≥ 3 jours, comptée exactement comme Stats : toute session + jokers ; rien si une session ou un joker couvre déjà aujourd'hui), 3. examen dans 7 jours, 4. premier démarrage 48–72 h après l'inscription sans vraie session (variante planning vide → Planning, ou déjà configuré → Chrono ; une fois), 5. second démarrage ~7 jours (une fois, seulement après le premier), 6. reprise après 7 jours sans vraie session (une fois par absence, 30 jours entre deux cycles), 7. reprise après 21 jours (après celle de 7 jours), puis plus rien jusqu'au retour. Vraie session = 10 min ou plus ; `studying_since` jamais lu. Examens = faits jamais plafonnés ; relances = 2 au plus sur 7 jours glissants, jamais la même deux soirs de suite, aucune le jour où le membre a étudié ; une relance bloquée laisse passer le type suivant. Textes personnalisés envoyés, modèles seuls au registre.
- Social, au moment de l'événement : demande reçue (« {prénom} veut t'ajouter 👋 »), demande acceptée (nouveau, une fois par amitié, seulement sur le passage réel en attente → acceptée), message privé (nouveau, sans le contenu, une notification par conversation toutes les 10 minutes + clé anti-doublon pour deux messages simultanés). Garde-fou social porté à 20 par 24 h. Liens : `/messages?tab=relations`, `?profile=<id>`, `?dm=<id>` (ajoutés à la page Social). Aucune présence « en train de regarder la conversation » fiable n'existe : pas d'optimisation inventée.
- v63 (déclencheurs `push_friend_accepted`, `push_private_message` via `push_webhook_call` : type + identifiant seulement ; plafond de relances 1–2, défaut 2), testée à blanc puis appliquée après le déploiement de la route. `supabase/tests/notifications_v1_triggers.sql` 10/10.
- Admin > Communications : Automatiques sépare « Programmées — rappel du soir » (ordre de priorité, fait/relance, exemple lisible) et « Social — au moment de l'événement » ; l'historique filtre Rappels / Social / Annonces et affiche la catégorie ; les modèles s'y lisent « [examen] demain », « [prénom] veut t'ajouter ». Profil et politique de confidentialité : Social = demandes reçues et acceptées + messages privés (sans leur contenu).
- Tests : `tests/notification-evening.test.mjs` (17 : examens demain / J+7 / plusieurs, série < 3 / ≥ 3 / joker / session courte / vraie session, activation J+3 / J+7 / une seule fois, reprise J+7 / J+21 / arrêt / remise à zéro / 30 jours, priorité, plafond, fuseaux) ; dispatch et social (demande acceptée une seule fois, message : pause, rafale, après 10 min, Social/Général coupés, suspendu, bloqué, aucun contenu au registre). Essai à blanc anonymisé sur les vraies données pour le 24/09 à 20 h : 22 membres évalués, 3 notifications (1 examen demain résumé, 1 série de 3 jours, 1 reprise à 7 jours).

## 2026-09-24 — Claude Code — Notifications : un seul chemin d'envoi, de vraies préférences, un centre de contrôle (phases A à E de l'audit)

- Sécurité et cycle de vie OneSignal : seul le compte qui a activé les notifications sur un appareil y est rattaché (`bt_push_owner` remplace `bt_push_enabled`, repris automatiquement). À la déconnexion l'appareil est détaché (`logout` OneSignal + registre `detached`) ; un détachement interrompu est terminé au lancement suivant. À la suppression du compte, l'utilisateur OneSignal et ses abonnements sont effacés (file `push_identity_cleanup` remplie par un déclencheur sur `auth.users`, vidée juste après la suppression puis chaque nuit). Webhook des demandes d'ami : secret de 256 bits dans Supabase Vault, vérifié par la base, demande relue (en attente, moins de 15 min) ; l'ancien secret faible n'ouvre plus rien. Une demande d'ami vers quelqu'un qui a bloqué l'expéditeur est ignorée en silence (aucune ligne, aucune notification). Retirés : `/api/push/announce` (plus appelé), `public/push/OneSignalSDKWorker.js` (jamais servi), `lib/pushServer.js`.
- Un seul chemin d'envoi (`lib/server/notify.mjs`) : audience décidée par la base (`notification_audience` : compte existant, non suspendu, interrupteur général, catégorie, blocage), fréquence, registre (`notification_sends` / `notification_recipients`), réservation anti-doublon par clé (un rappel par membre et par jour local : relancer le cron ne renvoie rien), OneSignal par identifiants de comptes en lots de 2 000, jamais par segment. Rappel du soir dans le fuseau du membre, jamais entre 22 h et 8 h chez lui ; plafond de relances sur 7 jours (3 par défaut, réglable de 1 à 7 ; l'examen du lendemain n'est pas plafonné) ; au plus 10 notifications sociales par 24 h.
- Préférences Général / Rappels / Social / Annonces dans le profil, appliquées côté serveur ; textes alignés sur ce qui est vraiment envoyé (aucun push promis pour les messages, réactions ou commentaires). Politique de confidentialité complétée (registre d'appareils, historique, durées) sans changement de version — à décider par Mathias.
- FR/EN : envois admin et annonces en français obligatoire + anglais facultatif (repli sur le français). Annonces datées (début, fin, heure de Bruxelles), ciblées (tout le monde ou une école), push facultatif. Les 4 annonces existantes n'ont pas été touchées.
- Admin > Communications : **Envoyer** (qui recevra, calculé par le serveur avec les mêmes règles que l'envoi ; exclusions ligne par ligne ; appareils joignables connus ; aperçu FR/EN ; « M'envoyer un test » ; confirmation), **Historique** (registre, filtres origine/période/auteur, livraison relue chez OneSignal à la demande, clics « non disponible »), **Automatiques** (dernier et prochain passage, chiffres, plafond, « Voir qui recevrait ce soir » à blanc), **Annonces**, **fiche membre** (préférences, appareils, vérification OneSignal en direct, dernier échec d'activation, derniers envois). Le MCP 21st.dev a été essayé ; aucun composant repris (il aurait importé un autre design).
- Base : v62 appliquée après test à blanc, avec la garde de suspension sur les 5 nouvelles tables. v62_1 (déclencheur des demandes d'ami avec le secret Vault) appliquée juste après la mise en ligne de la route qui le vérifie ; entre les deux, une demande d'ami ne part pas en push (la cloche reste juste). Conservation : destinataires 180 j, envois 1 an, appareils détachés 180 j, silencieux 1 an (purge de nuit).
- Vérifié : `supabase/tests/notifications_security.sql` 58/58 et matrice phase 1 92/92 sur la base réelle ; 232 tests Node (dont `notification-rules` et `notification-dispatch` : préférences, suspension, blocage, déconnexion, suppression, ciblage, bilingue, anti-doublon, fréquence, idempotence, sécurité des routes, essai à blanc) ; lint ; build ; commit exact rejoué dans une copie propre ; écrans hors ligne desktop/mobile, clair/sombre, FR/EN. Aucun push réel envoyé.
- Reste : phase F (état lu de la cloche synchronisé, push des messages privés), worker OneSignal auto-hébergé, 59 anciennes demandes d'ami en attente, mesure des clics OneSignal non vérifiée, premier rappel du soir inscrit au registre à contrôler.
...
- v61 (appliquée) : `admin_today`, `admin_members` (recherche sans accents, 7 segments, 6 tris, pagination serveur, `p_limit` null = export complet), `admin_member_detail`, `admin_activation` (entonnoir, cohortes par semaine lundi-Bruxelles, usage Planning/amis/salons). Une seule source de calcul (`admin_member_facts`, `admin_signup_cohorts`, non exécutables par l'app). Cohortes : valeur null tant que la fenêtre n'est pas complète, aucun taux sous 5 personnes. Admins exclus de tout (liste comprise), suspendus exclus des chiffres mais listés. Journal anonyme des suppressions : + retour en semaine 2, admin et suspendu au moment de la suppression. Index `sessions_user_started_idx`. v61_1 : seuil de 10 min verrouillé (alerte du linter).
- Vérifié : `supabase/tests/admin_analytics.sql` 88/88 (jeu daté 2000 rejoué via `blocus.analytics_now` : bornes exactes 599/600 s, 168 h, 336 h, jours et semaines de Bruxelles au changement d'heure, suppressions, petites cohortes, pagination, recherche, refus membre/suspendu/anonyme, aucun email) ; matrice phase 1 92/92 après v61 ; comparaison ligne à ligne des 287 comptes et de chaque indicateur avec des requêtes indépendantes en production : 0 écart. Temps de réponse 40 à 220 ms.
- Aucune interface modifiée (phase 3).
- Anomalies notées : 48 sessions de plus de 8 h (625 h, 16 % du temps réel total ; 17 anciennes dépassent le plafond actuel de 12 h), dont une de 11 h cette semaine (41 % des 26,8 h) — signalées à part, pas exclues ; 203 sessions de moins de 10 min (10,6 h) ; 32 comptes sans profil (10 ont des cours, 26 sessions) ; 57 adresses provisoires ; 31 suppressions antérieures à v58 sans semaine d'inscription ; 7 paires de vraies sessions qui se chevauchent (5 membres) ; 1 session antérieure à la création du compte ; 218 inscriptions sur la seule semaine du 18 mai, 2 par semaine depuis septembre.

## 2026-09-23 — Claude Code — Admin, phase 1 : fondations de sécurité et de modération

- Base (v55 à v60, appliquées et testées) : journal d'audit en ajout seul ; rôle admin et suspension non modifiables depuis l'app (`set_admin_role` réservé à l'éditeur SQL, même le service role ne peut pas l'exécuter) ; suspension réelle (plus aucune écriture, contenus masqués, sortie des classements, plus de nouveau contact) ; suppression de compte par le serveur avec effacement des fichiers d'abord ; les admins ne lisent plus les salons ni les publications privées, seulement le contexte d'un signalement ouvert (message + 2 avant + 2 après) ; table des passages des tâches planifiées. Matrice de permissions `supabase/tests/admin_phase1_security.sql` (membre, suspendu, admin, serveur, propriétaire) : 92/92 sur la base réelle ; suites existantes toujours vertes (activité 14, espaces 46, espaces par défaut 26, rapprochement 38, cas purs 184).
- Serveur : garde admin unique `lib/server/adminAuth.js` (admin non suspendu) pour les 6 routes existantes + 3 nouvelles (`/api/admin/members/[id]` suppression avec pseudo à retaper, `/suspension` avec bannissement Supabase Auth, `/moderation` : pseudo neutre, bio, photo). Motif obligatoire partout. Envois et annulations de notifications, réglages des automatiques, nettoyages de fichiers et ouverture d'une pièce jointe signalée sont tracés. Les suspendus ne reçoivent plus aucune notification (annonces, ciblées, rappel quotidien). Rappel quotidien et purge inscrivent leurs passages (conservés 90 jours). `lib/server/userFiles.js` partagé avec la suppression par soi-même.
- App : un compte suspendu voit « Ce compte est suspendu » à la connexion ; une session déjà ouverte est refermée. Recherche, suggestions et invitations de groupe excluent les suspendus. Photo du fil signée seulement si la publication est lisible par la personne (règles de la base). Retrait d'un contenu d'autrui par un admin : confirmation + fonction tracée.
- Ancien écran admin rebranché en attendant la phase 3 : « Éditer » devient « Modérer le profil » (3 gestes), « Message » retiré, Suspendre/Supprimer passent par les routes serveur ; lien « Nouvelles suggestions » et recherche réparés ; l'onglet Technique ne relance plus la même requête en boucle après une erreur (~200 appels constatés). Signalements : « Voir le contexte » et « Ouvrir la pièce jointe ». Supprimés : `AdminDemoStats.js`, la boîte de réception admin en double sur `/feedback`.
- Tests : `tests/admin-moderation.test.mjs`, suspension dans `tests/auth-login-identity.test.mjs` (158/158). Vérifié hors ligne : fenêtres suspendre/modérer/supprimer (motif, pseudo), contexte d'un signalement, déconnexion d'un compte suspendu, retrait d'une publication d'autrui.
- Reste pour la phase 2 : recalculer d'abord la base de référence avec la vraie session ≥ 10 min (première session, 2 et 5 jours distincts, activation 168 h, retour J7-J14), puis les fonctions d'analyse. Phase 4 : supprimer `admin_delete_user` et resserrer les droits restants.
...
- **Heatmap decalee d'un jour (BUG DE DONNEES)** — `components/StudyHeatmap.js` melangeait deux referentiels : les cles de sessions venaient de `started_at.slice(0,10)` (date **UTC**) et les cles de cases de `date.toISOString().slice(0,10)` sur un objet a **minuit local**. En UTC+2, minuit local = la veille 22:00Z, donc la case "aujourd'hui" portait la cle de la VEILLE : chaque case affichait le jour precedent et le travail du jour n'apparaissait jamais (la seule case avec la bonne cle etant marquee `isFuture`). Mesure avant correctif : aujourd'hui = 2026-07-26, cle de la case du jour = 2026-07-25. Les deux passent desormais par `localISO()` — la meme fonction que `computeStreak`, qui etait deja correct (d'ou l'incoherence serie/heatmap). Touche tous les utilisateurs FR/BE toute l'annee.
- **Ligne "moi" du classement illisible en sombre** — `components/Leaderboard.js:344` codait `backgroundColor: "#EAFBF4"` en dur alors que le texte herite `var(--bt-text-1)` (= `#F0EDE8` en sombre). Contraste mesure : **1,09:1** (WCAG AA exige 4,5). Remplace par `var(--bt-accent-bg)`, dont la valeur en clair est EXACTEMENT `#EAFBF4` (zero changement visuel en clair) et `#0D2B22` en sombre → contraste **12,98:1**. NB : les ~17 autres `#EAFBF4` en dur epinglent aussi `color: "#0E8F68"`, donc ils restent lisibles dans les deux themes (probleme de coherence visuelle, pas de lisibilite) — non touches.
- **Champs de saisie < 16px → zoom iOS force** — en dessous de 16px iOS Safari zoome au focus, et en PWA installee l'utilisateur ne peut plus dezoomer. Deux sources coexistaient : la classe `.input` (`font-size: 0.875rem`) et `text-sm` pose directement sur des inputs (le champ note du chrono n'utilise meme pas `.input`). Regle ajoutee dans `styles/globals.css`, ciblant les elements eux-memes sous 1024px (specificite (0,3,1) > utilitaires Tailwind), checkbox/radio/range exclus. Le desktop garde sa densite.
- **`xs:` etait une classe MORTE** — `tailwind.config.js` ne definissait aucun breakpoint custom et Tailwind 3 demarre a `sm:640px`, donc le `xs:flex-row` de `pages/dashboard.js:1288` ne s'appliquait jamais : **Pause** et **Terminer & enregistrer** restaient empiles pleine largeur a 10px d'ecart pendant une session. Breakpoint `xs: 380px` declare. Seuil choisi a 380 et non 400 : les iPhone les plus repandus font 390-393px (12/13/14 = 390, 15/16 = 393), un seuil a 400 les aurait tous exclus ; sous 380 (SE/mini = 375) les boutons restent empiles.

Verifie en preview offline (compte rempli : 143 sessions, serie 12 j). Heatmap : 53 colonnes, derniere case (aujourd'hui) coloree a `rgba(20,184,133,0.7)` = niveau 60-120 min, coherent avec les 1h25 affiches en haut, aucune case `transparent` dans la semaine courante. Classement en sombre : fond `rgb(13,43,34)`, texte `rgb(240,237,232)`, contraste 12,98:1, confirme visuellement. Inputs : tous a 16px a 390px, zero declencheur de zoom restant. Boutons : a 390px `flex-direction: row`, Pause 150x68 et Terminer 148x68 cote a cote sans debordement horizontal (et visuellement distincts — clair vs noir — ce qui limite le mé-tap). `npm run lint` clean, builds offline + normal OK.

Non traite volontairement (decisions produit en attente) : vue par defaut du planning sur mobile, thème par defaut `system` (le defaut est `light` en dur dans `pages/_document.js:19`, donc un iPhone en sombre recoit une app blanche), exposition ou suppression de `/historique` (page complete mais aucun lien navigable).
...
## 2026-09-18 — Claude — L'activité redevient un releve d'etude, pas un fil social
Deux registres au lieu d'un seul : une session ordinaire est desormais UNE LIGNE (qui a etudie, quel cours, combien de temps, quand), et un accomplissement — badge, niveau, serie — occupe la place qu'il merite avec le vrai objet du produit (le badge dessine, la flamme, le nouveau sceau de niveau partage avec Progression). Plus de carte blanche autour de chaque evenement.
Les sessions d'un meme etudiant dans une meme journee sont regroupees ("Tom a etudie 2 h 20 · Droit civil · Economie politique"), les sessions de moins de 20 minutes ne sont plus annoncees (elles comptent toujours partout ailleurs), et un objectif recurrent n'est annonce qu'une fois par semaine. Le reglage de partage automatique, invisible jusqu'ici (1 compte sur 251 l'avait active), devient une invitation de deux lignes en haut de la page.
Correctifs : le blocage s'applique enfin a l'activite (dans les deux sens, cote serveur), un echec de chargement ne s'affiche plus comme "aucune activite", les dix emojis de reaction deviennent un seul "Bravo", et la table morte post_views (0 ligne, aucun code) est supprimee. schema.sql documentait encore des regles RLS fausses : corrige.
Tests : 10 tests de grammaire + 9 de partage automatique + 14 verifications de securite SQL (annulees), lint, builds production et offline, verification navigateur en 320/390/1280, clair et sombre.

## 2026-09-18 — Claude — Profil : garder les deux cartes, retirer la coque de reseau social
Le profil gardait ce qu'il a de plus fort (la carte Progression avec son chien, la carte Badges) et perd ce qui le faisait ressembler a n'importe quel profil : plus de banniere decorative, un avatar plus petit, plus de pastille de niveau a cote du nom (le grand chiffre est juste en dessous), une ligne d'identite courte ("Medecine · Bac 2 · UCLouvain" au lieu de "Premier cycle (Bachelor / Licence) · annee 2 · Universite catholique de Louvain"), et deux informations en texte (temps etudie, serie + gels restants) au lieu de trois tuiles dont le classement souvent vide. Sur iPhone, les deux cartes sont maintenant entierement visibles au premier ecran ; sur ordinateur, elles prennent toute la largeur au lieu d'une demi-colonne.
Bug corrige : le profil annoncait 13 badges sur 22 quand la page Badges en comptait 10, parce qu'il recalculait les regles sur le telephone (jours UTC, sans les periodes de blocus) et ajoutait son verdict a celui du serveur. Les deux ecrans lisent desormais la meme liste serveur (lib/badgeTruth), et le calcul de secours de l'XP ne compte plus que les badges vraiment attribues.
Aussi : la bulle quotidienne "Ta serie est sauvee" sous la carte disparait (le chien de la carte reste), "Mon activite" (doublon de Stats) devient un simple lien vers Stats, la "Boite a suggestions" admin qui menait a la meme page que "Ameliorer l'app" est fusionnee, la page Progression reprend exactement le langage de la carte (grand chiffre sur l'encre, barre d'XP sans graduations), les erreurs d'enregistrement ne s'affichent plus en "Erreur : <message technique>" en francais meme en anglais, et les couleurs des boutons photo et notifications passent par les jetons du theme.

## 2026-09-22 — Codex — Signup/onboarding Phase 1 : une seule verite de reprise

Le parcours est desormais un seul enchainement Account → You → University → Studies → Courses. Les deux premieres etapes restent sur `/signup` et les trois suivantes sur `/onboarding`, mais la progression ne redemarre plus. Les nouveaux comptes portent `onboarding_version=1` dans leurs metadonnees Auth ; `lib/onboarding.mjs` determine la premiere etape incomplete depuis le profil serveur et les cours actifs. Les anciens comptes sans version restent volontairement legacy et ne sont pas bloques pour un `broad_field` manquant.

Les deux resultats de `supabase.auth.signUp` sont couverts : session immediate, ou ecran explicite de confirmation email lorsque `session` est nul. Le referral et les versions legales restent en attente dans les metadonnees Auth puis sont rejoues une fois la session disponible. L'onboarding de reparation conserve le meme UUID et devient lui aussi reprenable sur un autre appareil.
...
L'invitation iPhone abandonne la fausse capture Safari, le message App Store, la troisieme etape et les deux gros boutons de sortie. La feuille reprend la retenue de l'invitation notifications sans la copier : le nom complet Blocus Tracker, une phrase, l'icone Partager et une seule ligne compacte qui reproduit l'option Safari « Sur l'ecran d'accueil » / “Add to Home Screen”. La composition a ete legerement agrandie apres verification sur un vrai iPhone; les deux reperes a reconnaitre utilisent maintenant le vert et la surface menthe de Blocus Tracker. La croix, le clic hors feuille et Echap sont les seules sorties ; aucune mention des notifications n'entre dans ce parcours.

Le flux est desormais unique dans `_app` : quand `beforeinstallprompt` existe, le bouton Installer appelle directement l'invite native au lieu d'afficher les instructions Safari. L'etat standalone couvre `display-mode: standalone` et `navigator.standalone`; une installation acceptee est memorisee. Une fermeture ne masque plus l'invitation que pour la session courante : sans cooldown ni plafond, elle revient aux sessions suivantes jusqu'a l'installation. L'ancien bouton manuel « C'est deja fait » ne peut donc plus supprimer le rappel a tort. Le prompt attend aussi la decision de confidentialite et reste absent des pages d'authentification/onboarding.

Verification : 5 tests unitaires des etats direct/iOS/session fermee/installe, lint et build production OK. Parcours navigateur en 320/393/1280, FR/EN, clair/sombre, sans debordement, erreur ni violation axe; fermeture de session, retour au prochain lancement, installation directe et suppression apres installation confirmes.
