...
| Level | **Resolved 2026-09-18:** the level seal is `components/LevelSeal` — a circular medallion, the “Niv./Lv.” identity above a readable numeral, one rim that is never an XP meter. Progression and Activity use it; `LevelPill` stays for inline metadata (`Lv. N` beside a name). No third drawing of the same object, no hexadecimal colors of its own. |
