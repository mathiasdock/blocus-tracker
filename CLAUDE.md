...
4. **Read DESIGN.md before UI work.** “Blocus makes studying tangible” is the visual source of truth: scale-aware study time, course identity, exam priority, real progress, shared reward objects and academic relationships. Use semantic `var(--bt-*)` roles for UI; saved course colors, artwork and bounded kind colors follow its explicit rules. Neutral controls stay neutral. Do not turn every section into a card or copy a historical pattern over the current rules. The consolidation backlog is not authorization for a global redesign. → `DESIGN.md` + `docs/UI.md`
...
| `DESIGN.md` | Before any visual decision; product language and gradual consolidation rules |
