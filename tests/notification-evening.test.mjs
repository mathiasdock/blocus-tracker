...
import {
  NUDGE_WEEKLY_CAP, examVars, formatExamTime, normalizeNudgeCap, planEvening, renderEveningContent, streakBefore,
...
const plan = (input) => planEvening({ now: NOW, members: members(), ...input });
const only = (result) => {
...
test("streak: counted exactly like Stats — short sessions and jokers keep it going", () => {
  // Stats (computeStreak) compte tout jour avec une session, même courte.
  const short = only(plan({ sessions: [1, 2, 3].map((n) => session(daysAgo(n), 120)) }));
  assert.equal(short.data.days, 3);
  // Un jour gelé par un joker compte pour la continuité.
  ...
  assert.equal(streakBefore(new Set(["2026-09-23", "2026-09-22"]), "2026-09-24"), 2);
});
...
test("time zones: the member's own day, and quiet hours in their own time", () => {
  ...
  assert.equal(planEvening({ now: NOW, members: members(), sessions: nightly }).entries.length, 0);
...
test("preferences, suspension and switched-off kinds are counted, never sent", () => {
  const streak = [1, 2, 3].map((n) => session(daysAgo(n)));
  const excluded = only(planEvening({ now: NOW, members: members({ reason: "category_off" }), sessions: streak }));
...
test("each member is planned alone: nothing leaks from one to another", () => {
  const result = planEvening({
    ...
    sessions: [1, 2, 3].map((n) => session(daysAgo(n), 1800, V)),
    exams: [{ user_id: U, name: "Droit", exam_date: "2026-09-25" }],
  });
