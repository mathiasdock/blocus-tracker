...
export function parseQuickObjective(raw, { courses = [], baseDateISO } = {}) {
  ...
  // A minute-based duration also disambiguates a following afternoon hour:
  // "45min demain 14h" must not become a 14h45 study block.
  const hasMinuteDuration = /\b\d{1,3}\s*(?:min|mins|minutes?|m)\b/.test(work);
  if (!time && (bareHours.length >= 2 || (bareHours.length === 1 && bareHours[0].h >= 13 && hasMinuteDuration))) {
