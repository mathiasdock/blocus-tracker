...
export const offlineSupabase = {
  ...
  rpc(name, params = {}) {
    ...
    if (name === "get_my_promo_emails") {
      // Escape hatch de test : simule une prod où la migration v38 n'a pas
      // encore été exécutée, pour vérifier que le réglage se masque au lieu
      // d'afficher un état faux. Même pattern que le leaderboard v27.
      if (localStorage.getItem("bt_force_missing_promo_consent") === "1") {
        return Promise.resolve({ data: null, error: { message: "function get_my_promo_emails does not exist", code: "PGRST202" } });
      }
      const db = readDb();
      const profile = (db.profiles || []).find((row) => row.id === OFFLINE_USER_ID);
      return Promise.resolve({ data: profile?.promo_emails === true, error: null });
    }
