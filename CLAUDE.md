...
4. **CSS variables only for colors.** Use `var(--bt-*)` and the accent green `#14B885` family. No new colors. Dark mode works automatically. → `docs/UI.md`
   **Trois exceptions, toutes voulues et documentées :** la mascotte garde ses couleurs de pelage naturelles ; les types d'espaces d'étude de Communities (domaine, programme, cours, examen) ont chacun une teinte `--bt-kind-*` qui ne colore QUE leur pictogramme et le mot du type, jamais une surface, un état ou une action (demandé par Mathias le 2026-09-14 — `DESIGN.md` § Study-Space Kinds) ; et les badges sont des OBJETS DESSINÉS avec leur propre palette d'illustration (`lib/badgeArt.js`, 9 teintes, insensible au thème). Demandé par Mathias le 2026-09-09 : des badges tout-verts qui ne se distinguaient que par la profondeur du fond ne se distinguaient pas. Ne pas les re-verdir, ne pas les remettre dans des tuiles teintées. L'interface, elle, reste sur les tokens `--bt-*` — les icônes de réglages sont en encre neutre, sans pastille. → `DESIGN.md` § Illustration Palette
5. **Migrations : Claude les écrit ET les applique.** Changé par Mathias le 2026-09-07 (avant : exécution manuelle). Les fichiers de `supabase/` restent la trace écrite ; l'application se fait via le MCP Supabase (`apply_migration`), jamais par `psql` ni `supabase db push`. Deux règles tiennent : (a) **vérifier le schéma réel en base avant d'écrire** — `deleted_accounts` avait dérivé du dépôt, la migration v45 aurait échoué en silence ; (b) **demander avant toute étape irréversible** (suppression/anonymisation de données existantes), en annonçant le nombre de lignes touchées.
