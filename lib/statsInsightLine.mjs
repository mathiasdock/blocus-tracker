...
export function pickInsight(insights, { courseRows = [] } = {}) {
  ...
  // 4. Jour de semaine le plus étudié — en dernier, c'est le moins
  //    actionnable. Exige qu'il se détache nettement de la moyenne des autres
  //    jours. Le cumul ne dit pas une PRÉFÉRENCE : la phrase associée se
  //    contente donc de constater où le temps est allé.
  if (insights.topWeekday != null && insights.weekdaySecs) {
    const secs = insights.weekdaySecs;
    const total = secs.reduce((a, b) => a + b, 0);
    const best = secs[insights.topWeekday];
    const othersAvg = (total - best) / 6;
    if (othersAvg > 0 && best >= othersAvg * 1.6) {
      return { key: "topWeekday", vars: { weekday: insights.topWeekday } };
    }
  }
