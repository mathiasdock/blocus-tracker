...
/** Jours avec du temps étudié sur la période. */
export function activeDaysIn(days, range) {
  const set = new Set(
    daysInPeriod(days, range).filter((row) => Number(row.seconds) > 0).map((row) => row.local_date),
  );
  return set.size;
}
