...
//
// « Vraie session » = 10 minutes ou plus. La SÉRIE, elle, est la série
// OFFICIELLE (Phase 5A2) : la base la calcule avec le moteur canonique
// (public.study_streak_reminder_states → study_streaks) et la passe ici dans
// `streakStates`. À partir du 2026-10-05, la journée n'est validée qu'à 5 min
// cumulées ; un joker ou un jour hors blocus la couvrent aussi.
 ...
 * @param frozenDays Map(userId → ["YYYY-MM-DD"])   jokers utilisés (activation seulement)
 * @param streakStates Map(userId → { today, todayPreservesStreak, current }) —
 *                   série officielle vue par la base ; sans entrée, pas de rappel de série
...
export function planEvening({
  now, members, sessions = [], frozenDays = new Map(), streakStates = new Map(), exams = [], configured = new Set(),
  ...
  for (const [userId, member] of members) {
    ...
    // Série officielle : seulement si la base parle bien du même « aujourd'hui »
    // que ce plan (même fuseau) et qu'aujourd'hui ne préserve encore rien.
    const streakState = streakStates.get(userId);
    if (streakState && streakState.today === cal.today && !streakState.todayPreservesStreak
      && streakState.current >= STREAK_MIN_DAYS) {
      candidates.push({ kind: "streak_at_risk", data: { days: streakState.current } });
    }
