...
// dernier examen), avec un éventuel objectif d'heures pour les anciennes périodes.
// Les nouvelles périodes restent volontairement simples : deux dates suffisent.
// Elle remplace la progression sans fin
...
// Extension explicite : importé aussi par les tests Node (tests/study-days.test.mjs).
...
export function pickCurrent(periods) {
  // Date locale de l'appareil, comme la carte qui l'affiche (computeProgress).
...
export function suggestFromExams(exams) {
  // Date locale de l'appareil : à 21 h à New York, la date UTC est déjà demain.
  const today = localISO(new Date());
  ...
  return { start_date: today, end_date: end, goal_hours: null, examCount: upcoming.length };
}
...
/**
 * Progression d'une campagne à partir des jours d'étude déjà chargés — les
 * lignes de session_days `{ local_date, seconds }` (lib/studyDays.mjs) : une
 * session qui passe minuit compte sur ses deux jours, chacun pour sa part.
 * `days` doit couvrir la période (le dashboard charge 90 jours).
 * → { phase, daysTotal, daysElapsed, daysLeft, hoursDone, goalHours, pct,
 *     activeDays, paceNeeded }
 *   phase : "upcoming" | "active" | "ended"
 */
export function computeProgress(period, days, minSecondsFor = () => 1) {
  if (!period) return null;
  // Date locale de l'appareil : elle choisit quel jour est aujourd'hui.
  const today = localISO(new Date());
  const { start_date: start, end_date: end } = period;
  const phase = today < start ? "upcoming" : today > end ? "ended" : "active";

  const inRange = (days || []).filter((row) => row.local_date >= start && row.local_date <= end && Number(row.seconds) > 0);
  const hoursDone = inRange.reduce((a, row) => a + Number(row.seconds || 0), 0) / 3600;
  // « Jours étudiés » : seuil de la date (modèle canonique), jamais un joker.
  const perDay = {};
  for (const row of inRange) perDay[row.local_date] = (perDay[row.local_date] || 0) + Number(row.seconds || 0);
  const activeDays = Object.entries(perDay).filter(([date, secs]) => secs >= minSecondsFor(date)).length;
...
/** Intervalles [start, end] : le moteur canonique (studyDayStates) neutralise le hors-blocus. */
