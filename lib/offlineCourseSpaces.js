...
// a room joined from search. `?demo=empty` shows the student with courses,
// no match and no joined space yet (the cold start).
...
export function prepareOfflineCourseSpaces(db, userId) {
  ...
  db.course_rooms = [
    { id: "offline-room-macro", offering_id: "offline-offering-macro", created_at: iso(-9 * DAY) },
    { id: "offline-room-methodo", offering_id: "offline-offering-methodo", created_at: iso(-2 * DAY) },
    { id: "offline-room-compta", offering_id: "offline-offering-compta", created_at: iso(-60 * DAY) },
    { id: "offline-room-stats", offering_id: "offline-offering-stats", created_at: iso(-5 * DAY) },
    { id: "offline-room-media", offering_id: "offline-offering-media", created_at: iso(-12 * DAY) },
    { id: "offline-room-adv", offering_id: "offline-offering-adv", created_at: iso(-3 * DAY) },
  ];
  const members = {
...
export function offlineCourseSpacesRpc(db, name, params, userId) {
  ...
  if (name === "join_course_room") {
    ...
    if (!room) {
      room = { id: `offline-room-${Date.now()}`, offering_id: params.p_offering_id, created_at: new Date().toISOString() };
