...
- **Identity.** A course space carries the student's personal course marker (a 10–12px dot in the saved `course.color`), and only when an active personal course is linked to it; a space joined through search stays neutral. The two default spaces built from the profile carry the institution's real logo when the project ships one (`lib/universities.js`, on a small paper plate so a drawn wordmark survives the dark theme), otherwise its initials in ink; a program space carries no mark at all, only the width of its neighbour's, with the institution as its secondary line. No generic course icon, no tinted tile, no invented pictogram, no kind glyph or kind color.
...
- **Conversation.** Oldest to newest, day markers, consecutive messages of one author grouped, own messages right. Bubbles 18px with a 6px tail (shared with Friends); circles (marker, unread count, icon buttons) use a full radius; an institution's logo plate uses a 6px radius, the smallest shape that still reads as a crest at 24px. Message actions appear on hover/focus with a pointer and on tapping a bubble on touch; destructive actions stay in menus.
