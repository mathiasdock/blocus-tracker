import { shareSavedSession } from "./autoShare";
...
//   - plafond de 16 h dépassé sur une date (hint « daily_cap », v78) → refus
//     définitif, la renvoyer n'y changera rien : retirée, date dans `date`
//   - autre erreur → conservée, banner reste visible
// Renvoie { synced, alreadyExists, rejected, failed, results }.
export async function flushPending(supabase, userId, opts = {}) {
  const onItem = opts.onItem;
  const out = { synced: 0, alreadyExists: 0, rejected: 0, failed: 0, results: [] };
  ...
  for (const item of items) {
    const payload = {
      ...
      // Le fuseau capturé au démarrage voyage avec la session dans la file :
      // envoyée des jours plus tard, ailleurs, elle garde ses jours d'origine.
    ...
    if (!error) {
      await shareSavedSession(supabase, data || payload);
      removeFromQueue(item.id);
      out.synced++;
      out.results.push({ status: "synced", item, data });
      onItem?.({ status: "synced", item, data });
    } else if (error.code === "23505") {
      await shareSavedSession(supabase, payload);
    ...
    } else if (error.hint === "daily_cap") {
      removeFromQueue(item.id);
      out.rejected++;
      const rejected = { status: "rejected", item, error, date: error.details || null };
      out.results.push(rejected);
      onItem?.(rejected);
    } else {
      out.failed++;
