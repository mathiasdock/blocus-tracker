import { prepareOfflineSpaces, ensureOfflineSpace, syncOfflineSpaces } from "./offlineStudySpaces";
...
class OfflineQuery {
  ...
  async execute() {
    const db = readDb();
    if (this.table.startsWith("study_space") || this.table === "community_messages") prepareOfflineSpaces(db);
...
function rpcResult(name, params = {}) {
  if (name === "sync_my_study_spaces" || name === "ensure_study_space") {
    const db = prepareOfflineSpaces(readDb());
    const data = name === "ensure_study_space" ? ensureOfflineSpace(db, params, OFFLINE_USER_ID) : syncOfflineSpaces(db, OFFLINE_USER_ID);
    writeDb(db);
