...
export function offlineCourseSpacesRpc(db, name, params, userId) {
  ...
  // Contexte d'un signalement ouvert : le message signalé, jusqu'à deux
  // messages avant et deux après — même périmètre que la fonction v59.
  if (name === "admin_course_report_context") {
    const open = (db.course_message_reports || []).some((row) => row.message_id === params.p_message_id && !row.resolved_at);
    if (!open) return error("No open report for this message");
    const target = (db.community_messages || []).find((row) => row.id === params.p_message_id);
    if (!target?.room_id) return error("Message not found");
    const ordered = (db.community_messages || [])
      .filter((row) => row.room_id === target.room_id)
    ...
    const rows = ordered.slice(Math.max(0, index - 2), index + 3).map((row) => {
      const author = (db.profiles || []).find((item) => item.id === row.user_id);
      const position = row.id === target.id ? "reported" : (ordered.indexOf(row) < index ? "before" : "after");
      return {
        message_id: row.id, message_position: position, author_pseudo: author?.pseudo || null,
        content: row.content, attachment_name: row.attachment_name || null, exam_date: row.exam_date || null,
        created_at: row.created_at, hidden: !!row.hidden_at,
      };
    });
    return { data: rows, error: null };
  }
