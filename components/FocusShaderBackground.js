...
// "Waves" — made with the 21st.dev Shader Builder. The shader body stays
// unchanged; Blocus Tracker's identity is supplied through u_colors below.
...
export default function FocusShaderBackground({ paused = false }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("[FocusShader] WebGL background unavailable.", error);
      return undefined;
    }
    ...
    gl.uniform4f(uniforms.transform, 3165.0, 3.40, 0.16, 0.0);
    ...
    const draw = () => {
      ...
      gl.uniform4f(uniforms.scene, width, height, elapsed * 0.75, 5.0);
      gl.uniform4f(uniforms.space, 0.10, -0.25, pointerX, pointerY);
      gl.uniform4f(uniforms.cursor, presence, 2.0, 0.04, 0.41);
    ...
    const onPointerMove = (event) => {
      ...
      targetY = (1 - (event.clientY - rect.top) / Math.max(rect.height, 1)) * 2 - 1;
      targetPresence = 1;
    };
    ...
    const resizeObserver = typeof ResizeObserver !== "undefined"
      ? new ResizeObserver(() => { resize(); if (reduceMotion) draw(); })
      : null;
  ...
  return (
    <div aria-hidden="true" className={`bt-focus-shader${paused ? " is-paused" : ""}`}>
      <canvas ref={canvasRef} />
      <div className="bt-focus-shader-veil" />
    </div>
  );
}
