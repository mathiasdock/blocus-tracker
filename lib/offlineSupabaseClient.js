import { offlineCourseMessageVisible, offlineCourseSpacesRpc, prepareOfflineCourseSpaces } from "./offlineCourseSpaces";
import { OFFLINE_NOTIFICATION_RPCS, offlineNotificationRpc } from "./offlineNotifications.mjs";
import { DAY_PARTS_VERSION, computeSessionDayParts, isValidSessionTimezone, sessionDays } from "./sessionDayParts.mjs";
import { exchangeLocalDate, validateExchange } from "./exchange.mjs";
...
const OFFLINE_AVATAR_FILES_KEY = "bt_offline_avatar_files_v1";

function readOfflineAvatarFiles() {
  try {
    return JSON.parse(localStorage.getItem(OFFLINE_AVATAR_FILES_KEY) || "{}");
  } catch {
    return {};
  }
}

function writeOfflineAvatarFiles(files) {
  try {
    localStorage.setItem(OFFLINE_AVATAR_FILES_KEY, JSON.stringify(files));
  } catch {
    // Offline fixtures are best effort; production storage is never routed here.
  }
}
...
function blobAsDataUrl(blob) {
  return new Promise((resolve, reject) => {
    ...
    reader.onerror = () => reject(reader.error || new Error("Offline avatar read failed"));
...
function makeSeedDb() {
  ...
  const db = {
    ...
    posts: [
      ...
      // Les deux registres de l'activite : des sessions ordinaires (groupees
      // par etudiant et par jour) et de vrais accomplissements.
      {
        id: "offline-act-session-1",
        user_id: friendUser,
        caption: null,
        image_url: TEXT_ONLY_ACTIVITY_IMAGE,
        visibility: "public",
        created_at: hoursAgo(7),
        activity: { version: 1, type: "session_completed", seconds: 4500, courseName: "Droit civil", courseColor: "#3b82f6" },
      },
      {
        id: "offline-act-session-2",
        user_id: friendUser,
        caption: null,
        image_url: TEXT_ONLY_ACTIVITY_IMAGE,
        visibility: "public",
        created_at: hoursAgo(4),
        activity: { version: 1, type: "session_completed", seconds: 3900, courseName: "Economie politique", courseColor: "#f59e0b" },
      },
      {
        id: "offline-act-session-3",
        user_id: otherUser,
        caption: null,
        image_url: TEXT_ONLY_ACTIVITY_IMAGE,
        visibility: "public",
        created_at: hoursAgo(5),
        activity: { version: 1, type: "session_completed", seconds: 2700, courseName: "", courseColor: "" },
      },
      {
        id: "offline-act-badge",
        user_id: otherUser,
        caption: null,
        image_url: TEXT_ONLY_ACTIVITY_IMAGE,
        visibility: "public",
        created_at: hoursAgo(8),
        activity: { version: 1, type: "badge_unlocked", badgeId: "marathon_day" },
      },
      {
        id: "offline-act-level",
        user_id: friendUser,
        caption: null,
        image_url: TEXT_ONLY_ACTIVITY_IMAGE,
        visibility: "public",
        created_at: hoursAgo(11),
        activity: { version: 1, type: "level_up", level: 6 },
      },
      {
        id: "offline-act-streak",
        user_id: otherUser,
        caption: null,
        image_url: TEXT_ONLY_ACTIVITY_IMAGE,
        visibility: "public",
        created_at: hoursAgo(19),
        activity: { version: 1, type: "streak", days: 12 },
      },
      {
        id: "offline-act-goal",
        user_id: OFFLINE_USER_ID,
        ...
        activity: { version: 1, type: "goal_completed", title: "Terminer les exercices du chapitre 4" },
      },
    ],
    user_blocks: [],
    likes: [
...
// ── Portions quotidiennes (miroir de v65) ────────────────────────────────────
// En production, deux déclencheurs de public.sessions fixent le fuseau puis
// écrivent session_day_parts ; ici, la même règle en JavaScript
// (lib/sessionDayParts.mjs, parité testée avec le SQL).
function offlineSessionOnInsert(db, row) {
  if (isValidSessionTimezone(row.timezone)) {
    return { ...row, timezone_source: "device", day_parts_version: DAY_PARTS_VERSION };
  }
  const profile = (db.profiles || []).find((item) => item.id === row.user_id);
  const timezone = isValidSessionTimezone(profile?.timezone) ? profile.timezone : "Europe/Paris";
  return { ...row, timezone, timezone_source: "profile", day_parts_version: DAY_PARTS_VERSION };
}

function offlineSyncDayParts(db, sessionIds) {
  const ids = new Set(sessionIds);
  db.session_day_parts = (db.session_day_parts || []).filter((part) => !ids.has(part.session_id));
  for (const session of db.sessions || []) {
    if (!ids.has(session.id) || !session.timezone || !session.day_parts_version) continue;
    for (const part of computeSessionDayParts(session)) {
      db.session_day_parts.push({ session_id: session.id, user_id: session.user_id, ...part });
    }
  }
}

// Vue session_days (v70) : dérivée à la lecture, jamais stockée.
function offlineSessionDays(db) {
  const partsBySession = new Map();
  for (const part of db.session_day_parts || []) {
    if (!partsBySession.has(part.session_id)) partsBySession.set(part.session_id, []);
    partsBySession.get(part.session_id).push(part);
  }
  return (db.sessions || []).flatMap((session) => sessionDays(session, partsBySession.get(session.id) || []));
}
...
function readDb() {
  ...
  try {
    ...
    if (raw) {
      ...
      // Comme la colonne réelle (not null default false) : sans cela, un
      // filtre .eq("locked", false) écarterait les profils de démonstration.
      db.profiles = (db.profiles || []).map((row) => ({ ...row, locked: row.locked === true }));
      return db;
    }
...
const COURSE_SPACE_TABLES = new Set(["community_messages", "course_room_members", "course_links", "user_blocks", "course_message_reports", "course_room_optouts"]);
const COURSE_SPACE_RPCS = new Set([
  "resolve_my_course_links", "confirm_course_link", "reject_course_link", "course_space_summaries",
  "search_course_spaces", "join_course_room", "leave_course_room", "post_course_room_message",
  "report_course_message", "admin_course_reports", "admin_resolve_course_report", "admin_course_report_context",
  "ensure_my_default_rooms", "join_default_room",
]);
...
class OfflineQuery {
  ...
  getRows(db) {
    return [...(db[this.table] || [])].filter((row) =>
      ...
      // Reads of course-room messages follow the same rule as the RLS policy.
      && (this.mode !== "select" || this.table !== "community_messages" || offlineCourseMessageVisible(db, OFFLINE_USER_ID, row))
    );
  }

  async execute() {
    if ((this.table === "session_day_parts" || this.table === "session_days") && this.mode !== "select") {
      // Comme en base : aucune écriture client, seule la synchronisation écrit.
      return { data: null, error: { code: "42501", message: "permission denied for table session_day_parts" }, count: null, status: 403, statusText: "Forbidden" };
    }
    const db = readDb();
    if (COURSE_SPACE_TABLES.has(this.table)) prepareOfflineCourseSpaces(db, OFFLINE_USER_ID);
    // Vue : recalculée à chaque lecture, comme en base (jamais écrite dans readDb).
    if (this.table === "session_days") db.session_days = offlineSessionDays(db);
    ...
    if (this.mode === "insert") {
      ...
      if (this.table === "sessions") rows = rows.map((row) => offlineSessionOnInsert(db, row));
      db[this.table].push(...rows);
      if (this.table === "sessions") offlineSyncDayParts(db, rows.map((row) => row.id));
      writeDb(db);
      ...
      db[this.table] = db[this.table].map((row) => {
        ...
        // Le fuseau d'une session est figé à sa création (déclencheur a01).
        if (this.table === "sessions") {
          next.timezone = row.timezone;
          next.timezone_source = row.timezone_source;
          next.day_parts_version = row.day_parts_version;
        }
        return next;
      });
      rows = db[this.table].filter((row) => ids.has(row.id));
      if (this.table === "sessions") offlineSyncDayParts(db, [...ids]);
      writeDb(db);
    ...
    if (this.orders.length) rows.sort((a, b) => {
...
function rpcResult(name, params = {}) {
  if (name === "save_my_exchange") {
    ...
    const host = String(params.p_university || "").trim();
    if (host) {
      const invalid = validateExchange({ home: profile.university, host, start: params.p_start, end: params.p_end });
      if (invalid) return Promise.resolve({ data: null, error: { message: `Invalid exchange: ${invalid}` } });
    }
    profile.exchange_university = host || null;
    profile.exchange_institution_id = host ? `offline-exchange-${host.toLowerCase().replace(/[^a-z0-9]+/g, "-")}` : null;
    profile.exchange_start_date = host ? params.p_start : null;
    profile.exchange_end_date = host ? params.p_end : null;
    for (const course of db.courses || []) {
      if (course.user_id !== OFFLINE_USER_ID) continue;
      const lastStudy = (db.sessions || [])
        .filter((session) => session.course_id === course.id && session.duration_seconds > 0)
        .sort((a, b) => String(b.started_at).localeCompare(String(a.started_at)))[0];
      const day = exchangeLocalDate(new Date(lastStudy?.started_at || course.created_at),
        lastStudy?.timezone || profile.timezone || "UTC");
      course.study_institution_id = host && day >= params.p_start && day <= params.p_end
        ? profile.exchange_institution_id : null;
    }
    writeDb(db);
    return Promise.resolve({ data: null, error: null });
  }
  // Centre de notifications (v64) : mêmes réponses que la base, calculées ici.
  if (OFFLINE_NOTIFICATION_RPCS.has(name)) {
    const db = readDb();
    const result = offlineNotificationRpc(db, name, params, OFFLINE_USER_ID);
    writeDb(db);
    return Promise.resolve(result);
  }
  if (COURSE_SPACE_RPCS.has(name)) {
    const db = prepareOfflineCourseSpaces(readDb(), OFFLINE_USER_ID);
    const result = offlineCourseSpacesRpc(db, name, params, OFFLINE_USER_ID);
    writeDb(db);
    return Promise.resolve(result || { data: null, error: null });
  }
  // Retrait d'un contenu par un admin (v59) : même effet que la suppression.
  if (name === "admin_remove_post" || name === "admin_remove_comment") {
    const db = readDb();
    const table = name === "admin_remove_post" ? "posts" : "comments";
    const id = name === "admin_remove_post" ? params.p_post_id : params.p_comment_id;
    const before = (db[table] || []).length;
    db[table] = (db[table] || []).filter((row) => row.id !== id);
    if (table === "posts") {
      db.comments = (db.comments || []).filter((row) => row.post_id !== id);
      db.likes = (db.likes || []).filter((row) => row.post_id !== id);
    }
    writeDb(db);
    if (before === db[table].length) return Promise.resolve({ data: null, error: { message: "Unknown content" } });
    return Promise.resolve({ data: { ok: true }, error: null });
  }
  if (name === "get_my_referral_stats") return Promise.resolve({ data: computeReferralStats(), error: null });
  if (name === "is_pseudo_available") {
    const wanted = String(params.p_pseudo || "").trim().toLocaleLowerCase("en");
    const db = readDb();
    const taken = (db.profiles || []).some((profile) => (
      String(profile.pseudo || "").trim().toLocaleLowerCase("en") === wanted
    ));
    return Promise.resolve({ data: Boolean(wanted) && !taken, error: null });
  }
  ...
  if (name === "sync_my_badges") {
    // Like award_badges_for_user in production, the award is WRITTEN to
    // user_badges before the list is returned. The shim used to return the
    // list only, so every screen reading the table (level fallback, badge
    // truth) saw an empty collection offline.
    const awarded = [
    ...
    const db = readDb();
    db.user_badges ||= [];
    const owned = new Set(db.user_badges.filter((row) => row.user_id === OFFLINE_USER_ID).map((row) => row.badge_id));
    awarded.filter((id) => !owned.has(id)).forEach((id) => db.user_badges.push({
      id: `offline-award-${id}`, user_id: OFFLINE_USER_ID, badge_id: id, earned_at: new Date().toISOString(),
    }));
    writeDb(db);
...
export const offlineSupabase = {
  ...
  auth: {
    ...
    signUp(credentials = {}) {
      offlineUser.email = credentials.email || offlineUser.email;
      offlineUser.user_metadata = { ...offlineUser.user_metadata, ...(credentials.options?.data || {}) };
      // QA hatch: `bt_offline_confirm_email = 1` answers like a project that
      // requires email confirmation (a user, no session), so the check-email
      // screen can be seen without a real inbox.
      let confirmFirst = false;
      try { confirmFirst = localStorage.getItem("bt_offline_confirm_email") === "1"; } catch {}
      if (confirmFirst) return Promise.resolve({ data: { user: offlineUser, session: null }, error: null });
      const session = { user: offlineUser, access_token: "offline-token", refresh_token: "offline-refresh" };
      setSession(session);
      return Promise.resolve({ data: { user: offlineUser, session }, error: null });
    },
    ...
    updateUser(attrs = {}) {
      // Same semantics as Supabase Auth: `data` is merged into the current
      // metadata, null removes a key, and listeners receive USER_UPDATED with
      // a new user object. Replacing the metadata wholesale and staying silent
      // hid the 2026-09-23 PUT /user loop offline.
      const session = getSession();
      const currentUser = session?.user || offlineUser;
      const nextMetadata = { ...(currentUser.user_metadata || {}) };
      Object.entries(attrs.data || {}).forEach(([key, value]) => {
        if (value === null) delete nextMetadata[key];
        else nextMetadata[key] = value;
      });
      offlineUser.user_metadata = nextMetadata;
      if (attrs.email) offlineUser.email = attrs.email;
      const nextUser = { ...currentUser, email: attrs.email || currentUser.email, user_metadata: nextMetadata };
      const db = readDb();
      ...
      if (session) {
        ...
        if (storageAvailable()) {
          try { localStorage.setItem(SESSION_KEY, JSON.stringify(nextSession)); } catch {}
        }
        setTimeout(() => listeners.forEach((cb) => cb("USER_UPDATED", nextSession)), 0);
      }
      return Promise.resolve({ data: { user: nextUser }, error: null });
    },
    signOut() {
      clearSession();
    ...
    from(bucket) {
      return {
        async upload(path, file) {
          if (bucket === "avatars" && file instanceof Blob) {
            try {
              const files = readOfflineAvatarFiles();
              files[path] = await blobAsDataUrl(file);
              writeOfflineAvatarFiles(files);
            } catch (error) {
        ...
        async remove(paths = []) {
          ...
          return { data: paths.map((name) => ({ name, bucket_id: bucket })), error: null };
        },
        getPublicUrl(path) {
          const publicUrl = bucket === "avatars"
            ? readOfflineAvatarFiles()[path] || `/offline-upload/${bucket}/${path}`
            : `/offline-upload/${bucket}/${path}`;
