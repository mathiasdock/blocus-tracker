...
// ⚠️ LEGACY (Phase 5A2) — PLUS LA SÉRIE AFFICHÉE. La série officielle vient du
// serveur (public.study_streaks via get_my_streak) et, en local, de
// lib/studyDayStates.mjs (lib/useOfficialStreak.js). Ce calcul — jour de
// début de session, joker +1 — ne sert plus qu'au repli local des MISSIONS du
// Chrono, qui doivent garder leur règle jusqu'à leur propre phase (5B), où il
// sera supprimé.
//
...
//
// `blocusRanges` (optionnel) : [[début, fin], …] des périodes de blocus
// déclarées. Un jour SANS session situé hors de toute période est NEUTRE — il
// ne casse pas la série mais ne la prolonge pas. L'app est saisonnière (97 des
// 107 étudiants actifs en mai, 9 sessions en juillet) : une série perpétuelle
// punissait celui qui travaille dur pendant le blocus puis s'arrête, c'est-à-
// dire le comportement correct.
//
// ⚠️ Strictement opt-in : sans période déclarée, la règle ne s'applique pas —
// sinon TOUT jour serait neutre et la série ne casserait plus pour personne.
export function computeStreak(sessions, frozenDays, blocusRanges) {
  const days = new Set(sessions.map(s => localISO(s.started_at)));
  if (frozenDays) for (const d of frozenDays) days.add(d);
  if (!days.size) return 0;
  const ranges = blocusRanges && blocusRanges.length ? blocusRanges : null;
  const outsideBlocus = (iso) => !ranges.some(([from, to]) => iso >= from && iso <= to);

  const today = localISO(new Date());
  let streak = 0, i = days.has(today) ? 0 : 1;
  // `scanned` borne la remontée : une longue traversée de jours neutres
  // (des vacances d'été entières) parcourrait sinon des années.
  let scanned = 0;
  while (streak < 366 && scanned < 1096) {
    scanned++;
    const d = new Date(); d.setDate(d.getDate() - i);
    const iso = localISO(d);
    if (days.has(iso)) { streak++; i++; continue; }
    if (ranges && outsideBlocus(iso)) { i++; continue; }  // jour neutre
    break;
  }
  return streak;
}
...
// computeStreak passait déjà par localISO : ces helpers doivent parler la même
// langue, sinon les jours ne tombent pas en face les uns des autres.
