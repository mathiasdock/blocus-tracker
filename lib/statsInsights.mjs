...
export function computeInsights(sessions, days = null, minSecondsFor = () => 1) {
  const now = Date.now();
  const empty = {
    hasData: false,
    sessionCount: 0,
  ...
  // Régularité : jours ÉTUDIÉS distincts cette semaine vs la semaine passée —
  // même définition que la série (seuil de la date, lib/studyDayStates.mjs).
  ...
  for (const day of Object.keys(dayTotals)) {
    if (dayTotals[day] <= 0 || dayTotals[day] < minSecondsFor(day)) continue;
