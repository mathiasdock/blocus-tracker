...
// A program's disc carries the initials of its own name: one letter when the
// name is one word ("Médecine" → M), two when it is made of several ("Gestion
// & management" → GM). Two letters of a single word would read as an
// abbreviation nobody uses.
export function programInitials(label = "") {
  const words = String(label).replace(/[^\p{L}\p{N}\s]/gu, " ").split(/\s+/).filter(Boolean);
  const strong = words.filter((word) => word.length > 2 && !SMALL_WORDS.test(word));
  const picked = (strong.length ? strong : words).slice(0, 2);
