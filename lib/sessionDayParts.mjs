...
// ── Vue canonique session_days (v70) ─────────────────────────────────────────
// Miroir de public.session_days pour le mode démo. Une session avec fuseau
// garde ses portions ; une session historique sans fuseau donne UNE ligne,
// durée entière, au jour de début selon la règle de compatibilité figée
// 'Europe/Brussels' (ce n'est pas un fuseau inféré), sans découpage à minuit.

export const LEGACY_DAY_TIMEZONE = "Europe/Brussels";

/** 'captured' | 'inferred' | 'legacy', ou null pour une source inconnue (comme en SQL). */
export function sessionProvenance(session) {
  if (!session?.timezone) return "legacy";
  if (session.timezone_source === "device" || session.timezone_source === "profile") return "captured";
  if (session.timezone_source === "inferred") return "inferred";
  return null;
}

/**
 * Lignes session_days d'une session.
 * @param {object} session  ligne de sessions
 * @param {{local_date: string, seconds: number}[]} parts  ses portions (session_day_parts)
 */
export function sessionDays(session, parts = []) {
  if (!session) return [];
  const base = { session_id: session.id, user_id: session.user_id, course_id: session.course_id ?? null };
  if (!session.timezone) {
    const start = new Date(session.started_at).getTime();
    const seconds = Math.trunc(Number(session.duration_seconds) || 0);
    if (!Number.isFinite(start)) return [];
    return [{ ...base, local_date: localDateIn(start, LEGACY_DAY_TIMEZONE), seconds, provenance: "legacy" }];
  }
  const provenance = sessionProvenance(session);
  return [...parts]
    .sort((a, b) => a.local_date.localeCompare(b.local_date))
    .map((part) => ({ ...base, local_date: part.local_date, seconds: part.seconds, provenance }));
}
