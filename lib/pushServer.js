...
async function postToOneSignal(targeting, { title, body, url, sendAfter } = {}) {
  ...
  // Envoi differe : OneSignal garde la notification en file et l'expedie a
  // l'heure dite. Tant qu'elle n'est pas partie, elle reste annulable — c'est
  // ce qui donne un vrai droit a l'erreur sur un envoi de masse.
 ...
 *
 * ⚠️ Ne respecte AUCUN refus : le segment vit chez OneSignal, il ignore nos
 * préférences. Passe par sendAnnouncement() pour toute annonce réelle.
 */
...
/**
 * Annonce produit / nouvelle de l'app, en respectant les refus enregistrés.
 *
 * DEUX CHEMINS, ET POURQUOI :
 *   • Personne n'a refusé → diffusion par SEGMENT, exactement comme avant.
 *     C'est le chemin qui atteint aussi les appareils abonnés sans jamais
 *     avoir été associés à un compte (OneSignal.login), que le ciblage par
 *     identifiant ne verrait pas. Aucune perte de portée.
 *   • Au moins un refus → ciblage par identifiants, en retirant les personnes
 *     concernées. On accepte alors de perdre les abonnements non associés :
 *     un refus explicite pèse plus lourd qu'un envoi de plus.
 *
 * Les comptes suspendus (v57) sont exclus comme un refus : dès qu'il en
 * existe un, l'envoi passe par le ciblage par identifiants.
 *
 * Si la table des préférences n'existe pas encore (migration v44 non passée),
 * on retombe sur la diffusion par segment : le comportement d'avant, jamais
 * un envoi de plus que ce qui se faisait déjà.
 *
 * @param {object} admin  client Supabase service_role
 */
export async function sendAnnouncement(admin, opts = {}) {
  if (!admin) return { ...(await sendBroadcast(opts)), scope: "segment" };

  const { data: optOutRows, error: optOutError } = await admin
    .rpc("push_opted_out_users", { p_channel: "announcements" });

  if (optOutError) {
    return { ...(await sendBroadcast(opts)), scope: "segment", optOutsHonored: false };
  }

  const optedOut = new Set((optOutRows || []).map((r) => r?.user_id).filter(Boolean));

  // Un compte suspendu ne reçoit plus d'annonce : il est exclu comme un refus.
  // Contrairement aux préférences, on ne retombe jamais sur la diffusion par
  // segment si la lecture échoue — elle atteindrait aussi les suspendus.
  const { count: suspended, error: suspendedError } = await admin
    .from("profiles").select("id", { count: "exact", head: true }).eq("locked", true);
  if (suspendedError) throw new Error(`Unable to list suspended accounts: ${suspendedError.message}`);

  if (optedOut.size === 0 && !suspended) {
    return { ...(await sendBroadcast(opts)), scope: "segment", optOutsHonored: true, optedOut: 0, suspended: 0 };
  }

  const recipients = [];
  for (let from = 0; from < 100000; from += 1000) {
    const { data, error } = await admin
      .from("profiles").select("id").eq("locked", false).order("id").range(from, from + 999);
    if (error) throw new Error(`Unable to list recipients: ${error.message}`);
    for (const row of data || []) if (row?.id && !optedOut.has(row.id)) recipients.push(row.id);
    if (!data || data.length < 1000) break;
  }

  const result = await sendPushToUsers(recipients, opts);
  return { ...result, scope: "targeted", optOutsHonored: true, optedOut: optedOut.size, suspended: suspended || 0 };
}
...
/** Annule une notification pas encore partie. Sans effet sur une deja expediee. */
export async function cancelPush(notificationId) {
  const appId = process.env.NEXT_PUBLIC_ONESIGNAL_APP_ID;
  if (!appId || !notificationId) throw new Error("OneSignal not configured");
  const res = await fetch(
    `${ONESIGNAL_API}/${encodeURIComponent(notificationId)}?app_id=${encodeURIComponent(appId)}`,
    { method: "DELETE", headers: onesignalHeaders() }
  );
  const data = await res.json().catch(() => ({}));
  if (!res.ok) throw new Error(`OneSignal error ${res.status}: ${JSON.stringify(data)}`);
  return { ok: true };
...
export async function listRecentPushes(limit = 12) {
  ...
  return (data.notifications || []).map((n) => ({
    ...
    // Programmee = heure d'envoi dans le futur et pas encore terminee. Ce sont
    // les seules encore annulables.
    scheduledFor: n.send_after && n.send_after * 1000 > now && !n.completed_at
      ? n.send_after * 1000 : null,
    ...
    // La cible, décrite sans phrase toute faite : la page l'écrit dans la
    // langue de l'admin. Segments OneSignal, sinon le nombre de membres visés.
    segments: n.included_segments || [],
    targeted: (n.include_aliases?.external_id || []).length,
    url: n.web_url || null,
  }));
}
