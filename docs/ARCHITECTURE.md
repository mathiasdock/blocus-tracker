...
| `NotificationContext` | Notification centre (bell list and its account-level read state) and navigation badges (feed, friend requests, messages, course spaces, groups) — one `notification_summary` read per poll, not pushed | `inbox`, `notificationUnreadCount`, `loadInbox`, `loadMoreInbox`, `markNotificationRead`, `markAllNotificationsRead`, `openNotification`, `feedCount`, `friendCount`, `totalCommunity`, `messageCount`, `groupCount`, `markSeen` |
...
- **In-app notification centre** (v64 — `components/NotificationCenter.js`, `contexts/NotificationContext.js`, pure logic in `lib/notificationCenter.mjs`): one list built by the database from where things already live — friend requests received (while pending), requests accepted, private messages (one row per sender, never the text), comments and reactions on my posts, in-app announcements (active, dated, targeted). Nothing is copied: the only added state is what the member has read (`notification_reads` + the "mark all as read" cutoff in `notification_inbox_state`), so read on the phone = read on the computer. A conversation already read in Messages is read in the bell too; a new message relights it. Desktop: a panel attached to the sidebar bell (Escape, outside click and Tab-away close it, arrow keys between rows); phone: `InboxSheet`. Grouped Today / Earlier, 20 rows then "Voir plus". Each row opens its destination and becomes read: `/messages?tab=relations`, `/messages?profile=<id>`, `/messages?dm=<id>`, `/feed?post=<id>` (the feed fetches that post even when it is older than its 24 h), or the announcement's internal link — an announcement without one unfolds instead. Never in the bell: XP, levels, badges, missions, leaderboard, study sessions, planning edits; group and course-space messages keep their tab counters. All navigation counters come from ONE `notification_summary` call every 2 minutes (it used to be 7 to 13 requests); the feed, space and group "last seen" dates stay on the device, as before. Visual base: "Notification Panel" by Layro (21st.dev), rebuilt on Blocus tokens. Nothing is sent to a phone.
...
- **Catalogue V1** (`lib/pushAutomations.mjs`, texts editable in the admin):
  - *Evening* (`lib/eveningPlan.mjs`, pure): one notification per member per evening, the first that applies — exam tomorrow (fact), exam in 7 days (fact, single-evening window, so before any nudge), streak at risk (≥ 3 days counted like Stats, no session today), first start 48–72 h after signing up (two variants: empty planning → Planning, already set up → Chrono; once), second start ~7 days (once, after the first), comeback after 7 days without a real session (≥ 10 min; once per absence, 30 days between cycles), comeback after 21 days (then nothing until they come back). Facts are never capped; nudges: 2 over 7 rolling days, never the same two evenings in a row, none on a day the member studied. Texts are personalised (exam name and time, streak length); the registry keeps the template only.
