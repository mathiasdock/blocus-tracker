...
export function createNotificationStore(db) {
  return {
    ...
    /**
     * Rappels déjà envoyés (registre) à ces membres depuis `since` : le plan
     * du soir en a besoin pour « une seule fois », le plafond et les cycles
     * de reprise. Une lecture impossible arrête le passage (jamais de relance
     * à l'aveugle).
     */
    async history({ userIds, since }) {
      ...
      for (let i = 0; i < userIds.length; i += 200) {
        const { data, error } = await db.from("notification_recipients")
          ...
          .limit(10_000);
        if (error) throw new NotifyError("history_failed");
        rows.push(...(data || []));
      }
      return rows;
    },

    /** Une notification de message récente pour cette conversation ? */
    async recentRecipientKey({ userId, kind, keyPrefix, since }) {
      const { data, error } = await db.from("notification_recipients")
        .select("id")
        .eq("user_id", userId)
        .eq("kind", kind)
        .in("status", ["queued", "sent", "scheduled"])
        .gte("created_at", since)
 ...
 *   contentFor(row) texte propre à un destinataire (examen, série) : les
 *                 destinataires au texte identique partent ensemble ; le
 *                 registre garde logContent (le modèle), jamais ces valeurs.
...
export async function dispatchNotification({ store, onesignal, spec, now = new Date() }) {
  const {
    ...
    audienceRows = null, dryRun = false, recordEmpty = false, contentFor = null,
  } = spec;
  ...
  const batches = [];
  if (contentFor) {
    const byUser = new Map(eligibleRows.map((row) => [row.user_id, row]));
    const groups = new Map();
    for (const userId of claimed) {
      const own = contentFor(byUser.get(userId));
      const key = JSON.stringify(own);
      if (!groups.has(key)) groups.set(key, { content: own, ids: [] });
      groups.get(key).ids.push(userId);
    }
    for (const group of groups.values()) {
      for (const part of chunks(group.ids, ONESIGNAL_BATCH)) batches.push({ content: group.content, part });
    }
  } else {
    for (const part of chunks(claimed, ONESIGNAL_BATCH)) batches.push({ content, part });
  }
  for (const { content: batchContent, part } of batches) {
    try {
      const response = await onesignal.send({
        externalIds: part,
        title: batchContent.title,
        body: batchContent.body,
        url: batchContent.url || null,
        sendAfter,
      });
