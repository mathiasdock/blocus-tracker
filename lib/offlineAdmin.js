...
const SENDS = [
  ...
  { id: "10000000-0000-4000-8000-000000000007", created_at: iso(-4 * DAY), source: "automation", category: "reminder", kind: "nudge_study", trigger: "cron:push_daily", target_type: "automation", target_label: null, title: TXT("C'est l'heure de réviser 📚", "Time to study 📚"), body: TXT("Lance une petite session avant la fin de la journée."), url: "/dashboard", langs: ["fr", "en"], status: "partial", targeted: 2400, excluded: { frequency: 3 }, eligible: 2397, sent: 2000, failed: 397, onesignal_batches: 1, error: "onesignal_unavailable", delivery: null },
];
...
function memberNotifications({ p_user }) {
  ...
  return {
    ...
    recent: [
      { at: iso(-2 * HOUR), category: "reminder", kind: "streak_at_risk", status: "sent", source: "automation", title: TXT("Ta série est en danger 🔥") },
      { at: iso(-26 * HOUR), category: "announcement", kind: "admin_message", status: "sent", source: "admin", title: TXT("Bonne session d'examens") },
      { at: iso(-4 * DAY), category: "reminder", kind: "nudge_study", status: "failed", source: "automation", title: TXT("C'est l'heure de réviser 📚") },
    ],
    generated_at: iso(0),
  };
}
...
const AUTOMATION_STATUS = () => ({
  ...
  lastRun: {
    ...
    details: {
      date: iso(-2 * HOUR).slice(0, 10), cap: 3, evaluated: 41, planned: 20, eligible: 17, sent: 15, failed: 0,
      skipped: { studied_today: 12, inactive: 8, quiet_hours: 1 },
  ...
  stats: {
    ...
    nudge_study: { sends30d: 12, sent30d: 2030, failed30d: 397, lastSentAt: iso(-4 * DAY), sent7d: 2000, failed7d: 397 },
...
const DRY_RUN = () => ({
  date: iso(0).slice(0, 10), dryRun: true, cap: 3, evaluated: 41, planned: 19, eligible: 16, sent: 0, failed: 0,
  skipped: { studied_today: 13, inactive: 8, quiet_hours: 1 },
  excluded: { category_off: 1, frequency: 2 },
  kinds: {},
  members: {
    exam_tomorrow: { total: 4, pseudos: ["chloe", "ines", "nathan", "zoe"] },
    streak_at_risk: { total: 9, pseudos: ["adam.k", "camille", "elias", "hugo", "lea_v", "lina", "noah_b", "sarah.m", "yanis"] },
    nudge_study: { total: 3, pseudos: ["manon", "rayan", "victor"] },
  },
});
    ...
    return {
      data: {
        automations: AUTOMATIONS.map((a, index) => ({
          key: a.key, category: a.category, label: a.label, trigger: a.trigger, vars: a.vars,
          defaults: { title: a.title, body: a.body, url: a.url },
          current: { enabled: index !== 2, title: a.title, body: a.body, url: a.url },
        })),
